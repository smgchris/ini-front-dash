...
class UserRow extends React.Component {
  ...
  onUpdate(id) {
    window.location.assign("#/customers/update-customer/" + id)
  }
  ...
  render() {
    ...
    return (
      ...
      <tr>
        ...
        <td>{user.username}</td>
        <td>{user.balance}</td>
        <td>{user.packs>0?formattedTime:'Never subscribed'}</td>
        <td>{user.status===0?<div className="badge badge-success">Active</div>:<div className="badge badge-warning">Deactivated</div>}</td>
        <td><Button outline className="mb-2 mr-2 btn-transition" color="primary" onClick={() => this.onUpdate(user.userId)}>Update</Button>
        </td>
      </tr>
